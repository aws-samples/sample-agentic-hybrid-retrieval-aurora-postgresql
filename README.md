...
> `evidence`, retrieval, agent context, citations, evaluation, or replay. Every
...
PostgreSQL catalogs + application-pool/request telemetry + optional CloudWatch
       ...
       Authoritative Evidence (evidence.*)
                  ...
                  v
       Search & Ranking queue (retrieval.search_index_queue)
                  |
                  | runtime Cohere embedding batches
                  v
Versioned documents + chunks (retrieval.*)
  B-tree exact/filter indexes | GIN FTS | GIN pg_trgm | HNSW pgvector
                  |
                  v
Canonical Search & Ranking SQL functions
  exact ID tier + (full text + vector + fuzzy -> weighted RRF) -> model rerank
                  |
          +-------+--------+
          |                |
          v                v
Runs, Citations & Audit (proof.*)     Agent tools / HTTP / MCP
          |                |
          +-------+--------+
                  v
...
| Participant name | SQL schema | Owns |
|---|---|
| **Authoritative Evidence** | `evidence.*` | Normalized live telemetry, relational constraints, and authoritative run relationships |
| **Search & Ranking** | `retrieval.*` | Rebuildable document versions, chunks, embeddings, indexes, search functions, traversal, and search index health |
| **Runs, Citations & Audit** | `proof.*` | Retrieval runs, candidate signals, stages, answers, citations, supervised actions, judgments, and evaluation metrics |

The participant names are explanatory labels; the schema names are the stable
SQL contract used in the labs. `evidence.*` remains authoritative; the versioned
`retrieval.*` index is one-way derived, rebuildable state. See [architecture.md](docs/architecture.md),
...
must be current and `evidence.evidence_items` must be empty before a participant
...
populate `evidence`, `retrieval`, or `proof`. Workshop Studio runs it during
...
The suite truncates `evidence`, `retrieval`, and `proof`, so it requires both
