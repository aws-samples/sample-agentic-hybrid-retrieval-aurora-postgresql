...
1. creates a run-scoped lab table on the participant's Aurora database;
...
6. projects only those measurements into run-derived records;
...
observation, and deterministic projections of measured telemetry.
...
| `seed/load.sh`, `seed/dump.sh` | Restore and production of a preloaded corpus | Deleted; replaced by schema-only packaging and live admission |
| `admission/fixture_payload.json` | Three-record canned admission bundle | Deleted |
| `admission/promote_pg_incident.py`, `admission/admit.sh` | Fixed-ID promotion of multi-terminal JSON captures | Deleted; replaced by in-process run-scoped projection and admission |
...
| Workshop Studio seed dump and packaged source archive | Preloaded participant database state | Bootstrap is schema-only; the archive is rebuilt from the frozen source commit |
