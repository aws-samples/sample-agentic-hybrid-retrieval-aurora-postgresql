...
`doctor` distinguishes an empty database awaiting Wave A from a ready Wave A
or Wave B corpus. A CloudWatch warning is not a reason to restart the lab.

## Wave A Does Not Prove The Hold
...
Restore the standard 12 hot-write requests, then rerun Wave A:
...
None. Continue with the Wave A receipt. Do not block or rerun the workshop
solely to obtain CloudWatch data.

## Wave B Starts Without Wave A
...
Run Wave A, build the cited canonical agent answer, review its proposal, and
then run Wave B with that proposal ID. Wave B has no standalone incident
meaning.
...
correct the DDL and rerun Wave B.
```
...
rerun the same Wave B command. Do not delete the failed execution record.
...
recorded and no validation wave is admitted.
...
Rerunning Wave B returns an `idempotent_replay: true` admission receipt and
does not add documents or embeddings.

**Recovery**

Do not recreate or drop a matching index. The first ready Wave B capture is
already valid. A repeated Wave B run is safe and records a separate,
...
Editor as `workshop_participant`; let the owner-side Wave B recorder capture
...
or retrieval corpus as a shortcut. If a Wave A admission has not completed,
repair the disposable operational workload and reproduce the participant's own
incident. If Wave A is already admitted, preserve it and repair only the
participant action before retrying Wave B.
