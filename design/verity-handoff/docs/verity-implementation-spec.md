...
Synthetic normalized operational records
          ...
          casework.* relational truth
                 |
                 | deterministic render + projection hash + outbox
                 v
        retrieval.* versioned projection
...
| `retrieval` | One-way derived, rebuildable, versioned search and relationship projection |
| `proof` | Historical retrieval, stage, answer, citation, and evaluation receipts |

The indexed projection uses base tables rather than a materialized view because
...
- Release target: canonical thread plus deterministic background records to
  reach approximately 12,000 documents.
...
`projection_hash`. It is an input contract, not the indexed search surface.
...
| Relation | Contract |
|---|---|
| `projection_outbox` | Source revision queued for projection |
| `projection_builds` | Build version, model space, counts, status, timing, and error |
...
document version =
  ...
  + embedding model ID + projection hash
...
3. The projector renders `casework.v_evidence_documents`.
...
`retrieval.v_projection_drift` detects:

- missing current documents;
- revision or projection-hash mismatches;
- current documents that are not ready;
- current documents for deleted evidence;
- missing ready embeddings.

`retrieval.assert_projection_ready()` fails closed unless every live source row
...
An absent arm contributes zero. Raw FTS, cosine, and trigram values are not
added to the fused score. `final_score` is weighted RRF before optional model
reranking.
...
| `GET /ready` | Database projection readiness |
...
| Method and path | Purpose |
|---|---|
| `GET /v1/diagnostics/projection` | Health, embedding spaces, distribution, drift, builds |
...
The public API reads the ready projection and proof state. It does not expose a
...
- `BEDROCK_SYNTHESIS_MAX_TOKENS=700`
...
```bash
.venv/bin/python backend/scripts/build_casework_projection.py \
  --load-casework \
  --background-documents 12000 \
  --provider bedrock \
  --embed-missing
```
...
- Projection drift is zero before readiness.
- Exactly one canonical signature exists for each search function.

### Delivery

- Participant core is completable inside 60 minutes.
- No live provisioning, connector build, or 12,000-vector generation is in the
  participant path.
...
- Doctor passed all hard database, schema, index, ACL, projection, API, and
...
5. Generate and review the frozen 12,000-document Cohere embedding cache and
...
| Path | Owner |
|---|---|
| `backend/app/` | FastAPI, projection, retrieval, rerank, tools, synthesis, proof |
