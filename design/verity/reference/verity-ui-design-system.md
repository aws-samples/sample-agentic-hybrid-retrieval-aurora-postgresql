...
6. **Illustrative numbers are labeled at the point of use**, not just in the footer — modeled search indexes get a dashed-border note naming the release gate that will replace them.
...
Two screens designed but not produced, using the same system: **ingestion & search index** (`is_current` versioning, embedding backlog, drift, re-embed on model change) and **degradation console** (Bedrock throttle → extractive fallback, citation-validation failure, INVALID index cleanup).
