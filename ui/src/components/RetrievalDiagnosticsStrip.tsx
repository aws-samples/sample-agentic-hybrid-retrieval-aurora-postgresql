 ...
 * row it came from.
 *
 * It no longer draws per-arm candidate bars. `RetrievalChannelMap` sits four
 * rows above it in the same stage and already prints each arm's count against
 * the same pool, beside the index that served it and the sentence saying when
 * that arm earns its place. The bars were the second printing of those three
 * numbers inside one stage, and the receipt in Rank is a third. What is left
 * here is what nothing else on the page reports: where the time went, the
 * fused score at rank 1, and whether the reranker ran.
 */
      ...
      <dl className="lab-diagnostics-figures">
        <div>
          ...
          {/* The figure above is the wall clock the caller waited on, and the
              ...
              are close but never equal, and a participant who adds them up
              should find that out here rather than conclude a number lied. */}
          <small>
