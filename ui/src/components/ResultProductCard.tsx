...
export function ResultProductCard({ product, imageSrc, rank, footer, onSelect, onHighlight }: {
  ...
  const real = Boolean(product.source_dataset);
  const content = <>
    ...
    <span className="result-product-copy"><span className="result-product-category">{formatCategoryKey(product.category_key)}</span><strong className="result-product-name" title={product.title}>{product.title}</strong><span className="reviewed-card-fact">{reviewedProductSummary(product)}</span><span className="result-product-rating" aria-label={product.rating != null ? `${product.rating.toFixed(1)} out of 5 from ${product.review_count} ${real ? "historical ratings" : "reviews"}` : "No ratings yet"}><Star size={14} aria-hidden="true" />{product.rating != null ? <><strong>{product.rating.toFixed(1)}</strong><span>({product.review_count.toLocaleString()})</span></> : <span>No ratings yet</span>}</span>{real ? <span className="result-product-source">Original listing · Historical ratings</span> : <><span className="result-product-price">{formatPrice(product.price_cents, product.currency)}</span><span className="result-product-stock">{formatAvailability(product.availability)}</span></>}</span>
  </>;
  return <article className={`result-product-card${real ? " source-result-product" : ""}`} data-product-id={product.product_id}
