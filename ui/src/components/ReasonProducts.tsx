...
export function ReasonProducts({ products, citations = [], title, description }: {
      ...
      {products.map((product, index) => {
        ...
        return <li key={product.product_id} style={{ "--product-reveal-delay": `${Math.min(index, 4) * 60}ms` } as CSSProperties}>
          <Link href={`/products/${product.product_id}`} className="reason-product-link">
            ...
            <div className="reason-product-copy"><strong>{product.brand} {product.model}</strong><span>{formatCategoryKey(product.category_key)}</span></div>
          </Link>
