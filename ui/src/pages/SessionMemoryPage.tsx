...
export function SessionMemoryPage() {
  ...
  return <div className="page pipeline-inspector session-memory">
    ...
    {!data ? <p className="memory-loading" role="status"><LoaderCircle className="memory-spinner" size={18} />Loading conversations and memory settings…</p> : <>
      ...
      <div className="memory-layout">
        ...
        <section className="memory-strategies" aria-labelledby="memory-strategies-heading"><div className="memory-section-heading"><h2 id="memory-strategies-heading">What AgentCore remembers</h2><span className="memory-small-label">Long-term memory</span></div>
          <div className="memory-strategy-picker" aria-label="Types of memory">{strategies.map((item) => <button key={item.type} type="button" aria-pressed={type === item.type} onClick={() => setType(item.type)}><span>{item.name}</span><small>{item.technical}</small></button>)}</div>
