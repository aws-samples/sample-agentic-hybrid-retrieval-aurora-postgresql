...
function Records({ records }: { records: MemoryRecord[] }) {
  return <div className="memory-records">{records.map((record) => <article key={record.id} className="memory-record">
    <div className="memory-record-meta"><span>Extracted memory</span>{record.created_at && <time dateTime={record.created_at}>{date(record.created_at)}</time>}</div>
