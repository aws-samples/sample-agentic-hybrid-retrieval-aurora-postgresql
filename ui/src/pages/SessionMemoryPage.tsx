...
export function SessionMemoryPage() {
  ...
  return <div className="page pipeline-inspector session-memory">
    ...
    <ol className="memory-flow" aria-label="How AgentCore Memory works"><li><span>01 · Conversation</span><strong>Save a preference</strong><p>Inspect the message saved for this user and session.</p></li><li><span>02 · Extraction</span><strong>Inspect what was kept</strong><p>Wait for an actual preference record. Saving a message does not finish extraction.</p></li><li><span>03 · A new session</span><strong>Recall, then verify</strong><p>Keep Alex’s preferences and search Aurora for fresh product evidence.</p></li></ol>
    ...
    {!data ? (error ? null : <p className="memory-loading" role="status"><LoaderCircle className="memory-spinner" size={18} />Loading conversations and memory settings…</p>) : <>
      ...
      <section className="memory-recall" aria-labelledby="memory-recall-heading">
        <div className="memory-recall-intro">
          ...
          <p>Once a preference record is visible, start a new session and ask about his monitor. The answer should recall his preference and cite freshly retrieved product records.</p>
