...
CREATE OR REPLACE FUNCTION retrieval.assert_projection_ready()
RETURNS jsonb
LANGUAGE plpgsql
AS $$
DECLARE
  health retrieval.v_projection_health%ROWTYPE;
  embedding_space_count integer;
BEGIN
  SELECT * INTO health FROM retrieval.v_projection_health;
  ...
  IF health.drift_issues <> 0 THEN
    RAISE EXCEPTION 'retrieval projection has % drift issue(s)', health.drift_issues;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'drift_issues', health.drift_issues,
