...
def main() -> int:
    ...
    with OUT_CSV.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(outstanding[0]) if outstanding else ["product_id"])
        writer.writeheader()
    ...
    lines = [
        ...
        "|---|---|",
        *[
            f"| {item['source_title']} | `{item['output_filename']}` |"
        ...
        "|---|---|---|---|",
        "| `catalog-3x2` | 1800x1200 PNG | 1200x800 WebP | catalog grid, rail, related |",
        "| `detail-1x1` | 1600x1600 PNG | 1200x1200 WebP | product page hero (flagships only) |",
        "",
    ...
    for domain in ("consumer_electronics", "running_fitness", "home_office"):
        ...
        for subcategory in sorted(grouped[domain]):
            ...
            lines += [f"### {subcategory}", "", "| Product | Subject | Output filename | Anchor |", "|---|---|---|---|"]
