...
One bounded, catalog-read-only retrieval capability over Aurora PostgreSQL. It
generates candidates three ways, enforces eligibility in SQL, fuses the arms,
reranks the bounded pool, and returns a receipt of what it did and what it
granted. Search appends retrieval and candidate receipts, so it is not
idempotent and needs write capacity for observability data. The full capability
contract is [`../SKILL.md`](../SKILL.md); this document does not restate it. It
only answers a question that file deliberately leaves open: what does it look
like for something other than this workshop's own UI to call the same contract?

## 2. What another agent gets to consume

The HTTP skill surface exposes four operations, one scope handle, and one
receipt:
...
it carries forward as `retrieval_scope_id` on the remaining skill operations.
It is a retrieval-capability handle, not an identity. Evidence and comparison
enforce the granted product window. Explanation uses the handle to address the
event, but this workshop does not bind event replay to a principal. The
`diagnostics` and `applied_filters` returned with a search are the receipt: what
...
provisioned. Section 5 is a documentation profile, written so the composition
story stays concrete instead of hand-waved, not a claim that it runs anywhere.
...
`description: "Catalog-read-only product retrieval over Aurora PostgreSQL full-text,
